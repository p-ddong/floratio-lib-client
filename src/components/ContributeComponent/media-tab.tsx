...
interface MediaTabProps {
  ...
  setExistingImages?: React.Dispatch<React.SetStateAction<ImageData[]>>
}
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setUploadError(
          `${file.name} is too large. Please select images under 5MB`
        );
        return;
      }
  ...
  return (
    <CardContent className="space-y-6 pb-6">
      <div className="space-y-4">
        <div>
          <h3 className="text-sm font-medium mb-4">Contributed Plant images</h3>

          {/* Existing Images Section (for updates only) */}
          {contributionType === "update" && existingImages.length > 0 && (
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-medium">Current Images ({existingImages.length})</h4>
                <p className="text-xs text-muted-foreground">These are the existing images • Click X to remove</p>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg">
                {existingImages.map((image) => (
                  <div key={image.id} className="relative group border rounded-lg overflow-hidden bg-background">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      className="absolute top-2 right-2 z-10 bg-background/80 hover:bg-background/90 opacity-0 group-hover:opacity-100 transition-opacity"
              ...
              <Alert className="mt-4">
                ...
                <AlertDescription>
                  <strong>Note:</strong> {`Removing existing images will delete them from the plant's gallery. This action`}
                  cannot be undone once you submit the update.
                </AlertDescription>
              </Alert>
            </div>
          )}
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            {`Upload high-quality images that clearly show the plant's characteristics.
